...
class Api
{
	...
	{
		file_put_contents(HARM_START_UP_BASE_PATH . '/files/record', 'ON');
	}

	public function stop()
	{
		file_put_contents(HARM_START_UP_BASE_PATH . '/files/record', 'OFF');
	}

	public function get_latest()
	{
		$dir = realpath(HARM_START_UP_BASE_PATH . '/files/export/');
	...
	public function reference( $id ) {
		$file = realpath(HARM_START_UP_BASE_PATH . '/files/export/references/'.$id);
		if (file_exists($file)) {
			echo '{"data":';
	...
	{
		if (file_exists(HARM_START_UP_BASE_PATH . '/files/recorded')) {
			$file = fopen(HARM_START_UP_BASE_PATH . '/files/recorded', 'r+');
			if (flock($file, LOCK_EX) && filesize(HARM_START_UP_BASE_PATH . '/files/recorded')) {
				echo fread($file, filesize(HARM_START_UP_BASE_PATH . '/files/recorded'));
				ftruncate($file, 0);
				flock($file, LOCK_UN);
			}
			fclose($file);
	...
	public function delete_all()
	{
		...
		$dir = realpath(HARM_START_UP_BASE_PATH . '/files/export/references');
